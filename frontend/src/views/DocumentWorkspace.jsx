...
export default function DocumentWorkspace({ document }) {
  ...
  const pdfUrl = document.type === 'application/pdf' ? `${document.fileUrl}#page=${currentPage}&toolbar=0` : document.fileUrl;
  ...
  return (
    <div className="flex flex-col h-full w-full overflow-hidden">
      ...
      {/* OCR Warning Banner */}
      {isLowQuality && (
        <div className="bg-amber-50 border-b border-amber-200 px-6 py-2.5 flex items-center shrink-0">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-1/2 h-full bg-gray-900 border-r border-gray-300 relative flex flex-col">
          {document.type === 'application/pdf' ? (
            <iframe src={pdfUrl} className="w-full h-full bg-white" title="Viewer" />
          ) : (
            <div className="w-full h-full overflow-auto flex items-center justify-center p-4">
              <img src={pdfUrl} alt="Document" className="max-w-full shadow-2xl bg-white" />
            </div>
          )}
        </div>
            ...
            {activeTab !== 'PIPELINE' && (
              <div className="pb-2 relative">
                ...
                <input 
                  ...
                  className="pl-9 pr-3 py-1 text-sm bg-gray-50 border border-gray-200 rounded-full focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 w-48"
                />
              </div>
            )}
          </div>
